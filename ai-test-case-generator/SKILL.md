...
description: Generate traceable candidate manual test cases from a confirmed requirement and test points, using an optional verified Git commit as implementation evidence and the local Markdown case schema as the generation contract. Use after human confirmation and historical-case association; never generate automation code or mix commits.
---

# Manual test-case generation

Read [case-schema.md](references/case-schema.md) and [test-data-contract.md](references/test-data-contract.md), including their test-case quality rules, the matching test points, association, and challenge artifacts. The primary input is the confirmed requirement; a verified commit is implementation evidence and must not replace the requirement. Run `scripts/generate_test_cases.py` to verify the applicable source identity and that points are confirmed.
...
Then write candidate `04-test-cases.md` according to `case-schema.md`, without applying a Confluence template. Write the task metadata once, then expand the confirmed test points into `TC01`, `TC02`, and subsequent actual cases. Every case remains `pending_human_review`, is traceable to its test point, requirement, and implementation evidence where available, and contains all mandatory case-level fields required by `case-schema.md`. Apply the quality gate in `case-schema.md` before presenting cases for human review. Record the human review decision in `05-case-human-review.md`, including adopted, modified, or discarded per-case decisions. Do not mark cases adopted automatically.

Case generation is append-only by revision. Preserve every original test case and its review/adoption state, including adopted, modified, and discarded cases. If regeneration is needed, create a new `generation_id`/revision under `revisions/<revision>/04-test-cases.md`, reference the source test-point revision, and assign new case IDs or explicit supersession links. Never overwrite an existing `04-test-cases.md`, change a prior case's state, or alter the original state graph.
